// Heres is another pattern build on the same principle as the previous one.
...
void main()
{
    ...
    float brightness = 0.;
    float c1 = perlin(fragCoord, (iTime)/6., 100.)*3.;
    float c2 = perlin(fragCoord+100., (iTime)/2., 20.)*0.9-0.7;
    float contrast = c1/c2;
    ...
    p1 = perlin(fragCoord, (iTime*v)/10., 100.1)*contrast;
    p2 = perlin(fragCoord, (iTime*v)/10., 210.9)*contrast;
    p3 = perlin(fragCoord, (iTime*v)/3., 50.2)*contrast;
